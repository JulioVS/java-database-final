...
public class Inventory {
   ...
   // - This field will represent the current stock level of the product at the
   // store.
   private Integer stockLevel;

   // 5. Add relationships:
   // - **Product Relationship**: Use @ManyToOne to link this inventory entry to a
   // product.
   // - **Store Relationship**: Use @ManyToOne to link this inventory entry to a
   // store.
   // - Use @JsonBackReference("inventory-product") to prevent circular references
   // during JSON serialization for the product field.
   // - Use @JsonBackReference("inventory-store") to prevent circular references
   // during JSON serialization for the store field.

   // 6. Use @JoinColumn for foreign key associations:
   // - For the 'product' field, use @JoinColumn(name = "product_id") to specify
   // the foreign key column.
   // - For the 'store' field, use @JoinColumn(name = "store_id") to specify the
   // foreign key column.
   ...
   // - Add getters and setters for 'id', 'product', 'store', and 'stockLevel'
   // fields.
   // - Example: public Long getId(), public void setId(Long id)
   // - Example: public Product getProduct(), public void setProduct(Product
   // product)
